...
namespace Clients {
	 ...
	 * 
	 * distance(origin -> vertice) + distance(vertice -> deposit)  >  Tmax
	 *
	...
	int filter(std::pair<float,float> origin, std::pair<float,float> deposit) {
		...
		for(unsigned i=0 ; i<=vertices.size() ; i++) {
			...
			if( (distance(origin, vertices[i].getCoord()) + 
	...
	double addedTime(std::vector<int> trip, int topPriorityGene, unsigned pos, 
									 std::pair<float,float> origin, std::pair<float,float> deposit) {
	
		double dist;		
		
		if(trip.size() == 0)
			dist = distance(origin, vertices.at(topPriorityGene).getCoord()) + distance(vertices.at(topPriorityGene).getCoord(), deposit);
		else
			if(pos == 0)
				dist = distance(origin, vertices.at(topPriorityGene).getCoord())
						 	+ distance(vertices.at(topPriorityGene).getCoord(), vertices.at(trip.at(pos)).getCoord())
							- distance(origin, vertices.at(trip.at(pos)).getCoord());
			else
				if (pos == trip.size())
